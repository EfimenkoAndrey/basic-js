 ...
 */
 function transform(arr) {
	...
	if (resArray.indexOf('--discard-prev') > -1) {
		if (resArray[resArray.indexOf('--discard-prev') - 1] !== arr[arr.indexOf('--discard-prev') - 1]) {
	...
	if (resArray.indexOf('--double-next') > -1) {
		if (resArray[resArray.indexOf('--double-next') + 1] !== arr[arr.indexOf('--double-next') + 1]) {
			resArray.splice(resArray.indexOf('--double-next'), 1, arr[arr.indexOf('--double-next') + 1])
		} else if (resArray.indexOf('--double-next') >= resArray.length - 1) {
			resArray.splice(resArray.indexOf('--double-next'), 1);
		} else {
			resArray.splice(resArray.indexOf('--double-next'), 1, resArray[resArray.indexOf('--double-next')+1]);
		}
	}
	if (resArray.indexOf('--double-prev') > -1) {
		if (resArray[resArray.indexOf('--double-prev') - 1] !== arr[arr.indexOf('--double-prev') - 1]) {
			resArray.splice(resArray.indexOf('--double-prev'), 1)
		} else if (resArray.indexOf('--double-prev') === 0) {
			resArray.splice(resArray.indexOf('--double-prev'), 1);
		} else {
			resArray.splice(resArray.indexOf('--double-prev'), 1, resArray[resArray.indexOf('--double-prev') - 1]);
		}
	}	
	return resArray;
}
